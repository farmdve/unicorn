...
typedef enum uc_hook_type {
    UC_HOOK_PRE = 0,
    UC_HOOK_POST = 1 << 31,
    UC_HOOK_INTR = 1 << 1,   // Hook all interrupt/syscall events
    UC_HOOK_INSN = 1 << 2,   // Hook a particular instruction
    UC_HOOK_CODE = 1 << 3,   // Hook a range of code
    UC_HOOK_BLOCK = 1 << 4,  // Hook basic blocks
    UC_HOOK_MEM_READ_UNMAPPED = 1 << 5,   // Hook for memory read on unmapped memory
    UC_HOOK_MEM_WRITE_UNMAPPED = 1 << 6,  // Hook for invalid memory write events
    UC_HOOK_MEM_FETCH_UNMAPPED = 1 << 7,  // Hook for invalid memory fetch for execution events
    UC_HOOK_MEM_READ_PROT = 1 << 8,   // Hook for memory read on read-protected memory
    UC_HOOK_MEM_WRITE_PROT = 1 << 9,  // Hook for memory write on write-protected memory
    UC_HOOK_MEM_FETCH_PROT = 1 << 10,  // Hook for memory fetch on non-executable memory
    UC_HOOK_MEM_READ = 1 << 11,   // Hook memory read events.
    UC_HOOK_MEM_WRITE = 1 << 12,  // Hook memory write events.
    UC_HOOK_MEM_FETCH = 1 << 13,  // Hook memory fetch for execution events
} uc_hook_type;
